...
export default function ArtistAlbumsSection({ id, artist }: ArtistAlbumsSectionProps): ReactElement {
  const token = useContext(CredentialContext) as Credential;
  const containerRef = useRef<HTMLElement>(null);
  const { data, isLoading } = useFetchSpotify<ArtistAlbums>(
    `https://api.spotify.com/v1/artists/${id}/albums?include_groups=appears_on`, token, { method: "GET" }
  );
      ...
      <ResponsiveSwiper containerRef={containerRef} isLoading={isLoading}>
        {data?.items.map((album) => (
          <Link to={`/album/${album.id}`} key={album.id}>
            <Card coverImage={album.images.at(1)?.url as string} title={album.name} description={`By ${album.artists.map(e => e.name).join(", ")}`} />
          </Link>
        ))}
      </ResponsiveSwiper>
    </Section>
